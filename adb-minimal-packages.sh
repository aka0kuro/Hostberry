#!/usr/bin/env bash
#
# Mantiene: Play Store, Wallet y lo necesario para Wallet (NFC, etc.).
# Elimina (deshabilita): YouTube, Drive, Gmail, Fotos, Maps y el resto de apps.
# Solo toca aplicaciones; el sistema no se modifica → evita bootloop.
#
# Reversible: adb shell pm enable <package>
# Si bootloop: ./adb-recovery-bootloop.sh
#
...
# Aplicaciones a eliminar (deshabilitar). Se mantienen Play Store, Wallet y lo necesario para Wallet.
# Incluye: YouTube, Drive/Docs, Gmail, Fotos, Maps, y el resto de apps opcionales.
DISABLE_LIST=(
