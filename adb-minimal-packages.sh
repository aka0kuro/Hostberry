#!/usr/bin/env bash
#
# Script CONSERVADOR: deja solo Wallet, System UI, Bluetooth, WiFi, NFC,
# Play Store, Pixel Launcher y lo mínimo para arranque.
#
# Solo DESHABILITA (nunca desinstala). Todo reversible con:
#   adb shell pm enable <package>
# o con adb-recovery-bootloop.sh si hay bootloop.
#
# Uso: dispositivo conectado por USB con depuración ADB:
...
# Whitelist: lo que queremos mantener + todo lo crítico para evitar bootloop
KEEP=(
    android
...
# Lista de paquetes del dispositivo
ALL_PACKAGES=(
...
echo "=== ADB Minimal (CONSERVADOR: solo deshabilitar) ==="
echo "Se mantienen: Wallet, System UI, BT, WiFi, NFC, Play Store, Launcher + críticos de arranque."
echo "El resto se deshabilita con pm disable-user (reversible)."
echo ""
...
echo ""
echo "Reinicia el dispositivo. Para revertir: adb shell pm enable <package>"
echo "Si hay bootloop: ./adb-recovery-bootloop.sh"
